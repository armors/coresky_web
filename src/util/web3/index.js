...
export default {
	...
	chainChanged(channelId) {
		...
		if (parseInt(channelId) !== parseInt(process.env.VUE_APP_CHAINID)) {
			tools.messageBox(
				i18n.global.t("global.errNetwork"),
				i18n.global.t("global.changeNetworkTo") + tools.networkName(process.env.VUE_APP_CHAINID)
			);
		}
	},
	disconnect(error) {
	...
	async changeNetwork(network) {
		...
		console.log(Web3.utils.hexToNumber(chainId), parseInt(process.env.VUE_APP_CHAINID))
		if (Web3.utils.hexToNumber(chainId) !== parseInt(process.env.VUE_APP_CHAINID)) {
			if (!network) {
