<template>
	<div class="userCardList">
		<div class="card-list" v-loading="isLoading">
			<div
				class="card-item"
				:class="{ active: item.pledge === 1 }"
				v-for="(item, index) in dataList"
				:key="index"
			>
				<img
					:src="require(`@/assets/core-card/v${item.level}.png`)"
					@click="goDetail(item)"
					class="card-img"
					alt=""
				/>
				...
				<div class="card-info">
					...
					<div class="card-vip">
						<img
							:src="
								require(`@/assets/core-card/vip${item.level}.png`)
							"
							class="vip-img"
							alt=""
						/>
			...
			<div class="card-item">
				<img
					src="@/assets/core-card/vip_m_0.png"
					class="vip-img-0"
					alt=""
				/>
...
export default {
		...
		user: {
			handler(val) {
				if (
					...
					this.$store.state.useAuthorization
				) {
					this.init();
				}
			},
	...
	methods: {
		...
		async bindCard(item) {
			for (let i = 0; i < this.dataList.length; i++) {
				if (this.dataList[i].pledge === 1) {
					this.$tools.notification(
						'',
						this.$t('messageTip.coreCardBindCheck'),
						'error'
					);
					return;
				}
			...
			item.isloading = true;
			const result = await ERC721Template.depositCard(
				item.tokenId,
				this.user.coinbase
			);
		...
		async unBindCard(item) {
			item.isloading = true;
			const result = await ERC721Template.withdrawCard(
				item.tokenId,
				this.user.coinbase
			);
			if (result && result.status === true && result.blockHash) {
